...
<div class="row  mb-5">
  <div class="col-12 mb-md-0  d-flex align-items-center justify-content-center gap-2 pt-2">
      <a href="{{ route('deficiency-filter', ['program' => 'All', 'service'=> $_service] ) }}" class="btn btn-sm btn-outline-dark  {{ ($_program == 0 ? 'active' : '') }}">All</a>
      @if(isset($programs) && count($programs) > 0)
        @foreach($programs as $program)
            @if($program)
            <a href="{{ route('deficiency-filter', ['program' => $program->program_name, 'service'=> $_service] ) }}" class="btn btn-sm btn-outline-danger  {{ ($_program == $program->id ? 'active' : '') }}">{{ ucfirst($program->program_name) }}</a>
            @endif
        @endforeach
      @endif
  </div>
